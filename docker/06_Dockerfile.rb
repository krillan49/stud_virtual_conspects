...
# 2. WORKDIR – указывает рабочую директорию в образе. Все команды в контейнере будут выполняться относительно этого каталога
...
# EXPOSE 80                   - декларируем что будем пробрасывать наружу порт 80
...
# 6.a. CMD – команда, что выполняется каждый раз при запуске контейнера (docker start / docker run) (?? нужно указать фаилы из WORKDIR рабочей директории образа ??). Выполняет команды через Shell, тоесть запускается оболочка /bin/sh (тоже что в верхней строке шелл скриптов)
