...
# 1. FROM – указывает какой образ будет использован(скачает из Докер Хаба если его у нас нет локально) как базовый, с которого мы начнем сборку нашего образа (например openjdk). Можно дополнительно указать через двоеточие "тег", тут версию Джавы (например :11), по умолчанию будет последняя версия. Можно после версии указать еще и сервер(например -apache).
...
# 3. COPY – указывает какие файлы (по их местоположению относительно расположения Dockerfile) из нашего проекта будут скопированы с хост машину в контейнер/образ. Принимает 2 параметра: путь к фаилам (относительно расположения Dockerfile), которые хотим скопировать, на нашем ПК (откуда копируем); путь контейнере (куда копируем)
