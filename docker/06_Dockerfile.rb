...
# LABEL – команда Dockerfile, при помощи которой мы можем указать любую дополнительную информацию о нашем образе, задать любые имена параметров и их значения
...
# WORKDIR – команда Dockerfile указывает рабочую директорию в образе. Все команды в контейнере будут выполняться относительно этого каталога. Это папка по умолчанию, в которой вы запускаете среду разработки. Моно назвать её как угодно.
...
# COPY – команда Dockerfile указывает какие файлы (по их местоположению относительно расположения Dockerfile) будут скопированы с хост машины в образ. Принимает 2 параметра: путь к фаилам (относительно расположения Dockerfile), которые хотим скопировать, на нашем ПК (откуда копируем); путь в контейнере (куда копируем)
...
# ENV - команда Dockerfile указывающая переменные окружения по умолчанию, например временную зону, какието айдишники, какието адреса на внешние сервисы итд. Команда принимает переменную окружения и через пробел или = ее значение. Если переменные окружения меняются, то лучше из задать в опциях команды run, через флаг -e, а не в Dockerfile
...
# 2. ENTRYPOINT - выполняет команды без использования Shell(/bin/sh). Потому команда из ENTRYPOINT фиксирована и будет выполнена в любом случае и ее невозможно перезаписать командой параметром если запустим контейер при помощи команды run
