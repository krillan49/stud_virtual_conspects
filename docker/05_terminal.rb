...
# $ docker images -q                      - айдишники всех образов, например чтобы потом удалить

# > docker ps                             - список запущенных контейнеров с их именами, id, образами, портами
# > docker ps -a                          - (all) полный список всех существующих локально контейнеров и запущены ли они
# $ docker ps -a -q                       - список айдишников всех контейнеров, например чтобы потом удалить
...
# $ docker rmi 5ecde7c66384                    - по айди образа
...
# run   - команда создает и запускает контейнер на основе указанного образа (если данного образа или образов его составляющих нет, то сначала их скачивает с ДоерХаб). Дополнительно выдаст сообщение если скачивает образ. По умолчанию будет каждый раз создавать новый контейнер с рандомным названием при запуске от образа, так что лучше задать название и потом запускать уже от контейнера через "start". Может принимать команду, которая выполнится в контейнере после запуска (заменяет CMD)
...
# -d (--detached)      - отсоединенный(фоновый) режим: мы возвращаем управление в наш терминал, а не оставляем его внутри контейнера, контейнер бует работать в фоновом режиме и терминал в котором его запускали не будет занят выводом логов его работы, а будет доступен для новых команд.
