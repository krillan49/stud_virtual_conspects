...
# -p (--publish list)  - опция команды run, позволяющая соединить порты, можно дополнительно указать какие порты соединяются если они разные. Например 8001:80, тут 8001 это порт связанный с нашим компом, а 80 это порт приложения пробрасываемый из контейнера. Те мы связываем по порту 8001 наш комп с портом 80 контейнера/проекта
