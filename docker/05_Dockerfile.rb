...
# 3. WORKDIR – указывает рабочую директорию в образе (?? откуда будет исполнться код)
...
# 5. RUN – описывает команду, что выполнится один раз при сборке (run или build ??) всего контейнера/создании образа (?? нужно указать фаилы из рабочей директории образа ??)
