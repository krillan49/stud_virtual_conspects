...
# Docker Hub - хранилище образов (один из регистров). Он является официальным и содержит большое количество образов



puts '                                        Авторизация в терминале'

# Перед логином в терминале нужно зарегаться на https://hub.docker.com/
...
# Официальные образы помечены зеленой галочкой и подписью, синей галочкой образы от подтвержденных пользователей

# pull - команда скачивает образ из Docker Hub.
...
# > docker pull openjdk:11         - конкретная версия (в докумментации в хабе указаны разные доступные версии)



puts '                                      Залив образа на Docker Hub'

# Чтобы наши образы могли скачать другие люди, нужно выгрузить их в репозитории на Docker Hub. Чтобы залить образ на Docker Hub нужно предварительно залогиниться локально. Можно заливать образы через аккаунт на Докерхаб во вкладке "Repositories" нажав кнопку "Create Repository" либо у себя локально собрать образ и запушить его на ДокерХаб при помощи команд

# 0. Если у нас много сервисов через docker-compose то нужно будет создать для каждого отдельный докерфаил, отдельно каждый сбилдить и запушить на Докерхаб и потом если будем использовать тот же docker-compose(например на сервер чтобы задеплоить), то переписать там во всех сервисах build на image, тк теперь у нас уже есть готовые образы.

# 1. Собираем образ для хаба, указав как название логин_на_докерхаб/имя_репозитория(название_образа) и путь к папке с Dockerfile относительно текущей директории
