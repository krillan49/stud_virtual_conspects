...
# Docker Volumes / Докер Тома – механизм для хранения данных, который позволяет сохранять данные контейнеров вне их файловой системы для сохранения состояния приложений и данных между перезапусками контейнеров. Сохраняет настройки на сервере(хост машине). За счет томов можно хранить значения в БД, файлы, документы итд. Том означает, что даже если мы закроем или удалим Docker, мы можем указать, какие файлы мы хотим сохранить на нашем локальном компьютере. Тоесть данные хранятся не в контейнере, а в Volume и соответсвенно при перезапуске контейнера он опять получит к ним доступ. Удобно хранить там например БД
...
# Этот способ удобен тем, что удобно заранее поместить в директорию на хост машине что угодно, например сделать доступной контейнеру наше давно написанное приложение
...
# :ro  - (рид онли) контейнер не имеет права изменять что-то в директории вольюма на хост машине, а мжет только читать
