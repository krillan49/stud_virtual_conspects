...
# (?? Потом проверить Хз как это работает, создает новые контейнеры, когда сервис апущен или нет ??):
...
# 2. up - команда запуска проекта, который был собран(build) на основе сервисов из docker-compose.yml, создает и запускает контейнеры для каждого образа проекта. При повторном запуске запустит тот же контейнер, что был ранее создан.
...
# - Тестирование и отладка отдельных сервисов в изолированной среде.
