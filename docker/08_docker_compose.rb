...
# docker-compose.yml - используется для описания всех связанных с проектом контейнеров, их запуска и взаимодействия между ними
...
# docker compose(С пробелом) - это новый проект compose на Go. Первоначальный проект compose на python, называемый docker-compose (команды были тоже с дефисом) устарел, и разработка перешла на v2. Чтобы установить v2 docker composeкак плагин CLI в Linux, поддерживаемый дистрибутив теперь может установить "apt-get install docker-compose-plugin"
...
# 1. build - команда позволит получить Dockerfile и установить всё остальное из docker-compose.yml. Установка может занять время, тк потребуется загрузить необходимые пакеты
...
# $ docker compose up service_name - запуск конкретного сервиса, а также все связанных с ним сервисов (указываются в depends_on в docker-compose.yml).
...
# 3. run - команда принимает имя сервиса из docker-compose.yml и команду, которая будет запущена внутри контейнера.
# --rm               - флаг, чтобы удалить контейнер после использования
# --service-ports    - флаг, чтобы использовать порт заданный в сервисе

# $ docker compose run web rake db:create                - сшздать в Рэилс базу данных в контейнере сервиса run
...
# 4. down - команда очистки, нужна, когда вы закончили работу над проектом и хотите удалить среду разработки, чтобы сэкономить место. Docker удалит все ваши настройки и загруженные образы.
