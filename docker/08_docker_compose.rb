...
# 1. build - команда позволит получить Dockerfile или стянуть образы и установить всё остальное из docker-compose.yml. ?? Или мб нужно только чтобы собрать образы ??

# $ docker compose build                 - сборка проекта(контейнеров) для всех сервисов описанных в фаиле docker-compose.yml
# $ docker compose build service_name    - сборка контейнера для конкретного сервиса из docker-compose.yml
# $ docker compose -f docker-compose.prod.yml build   - собрать на основе конкретного docker-compose фаила


# (?? в build ??) Предварительно скачаетвание и установит те образы которых нет в наличии


# 2. up - команда запуска собранного проекта, тоесть запуск всех контейнеров созданных на базе сервисов из docker-compose.yml.
# --build   - флаг команды up чтобы пересобрать образы перед тем, как запустить их. Удобно использовать вместо запуска команд "build" и "up" отдельно. Полезно, если внесли изменения в код приложения, Dockerfile или docker-compose.yml
...
# $ docker compose up some        - запуск конкретного сервиса some, а также все связанных с ним сервисов (указываются в depends_on в docker-compose.yml)
...
# 3. run - команда запускает новый экземпляр указанного сервиса, создавая каждый раз новый контейнер. Принимает имя сервиса из docker-compose.yml и дополнительную команду, которая будет запущена внутри контейнера. Это может быть полезно для:
# - Выполнение одноразовых команд (например, выполнить миграцию базы данных).
# - Запуск интерактивных оболочек (таких как `bash`) в контейнере.
# - Тестирование и отладка сервисов в изолированной среде.
...
# COMMAND - команда (и арументы для нее), которую нужно выполнить внутри контейнера (Если не указана, будет использоваться команда по умолчанию из `Dockerfile` или из `docker-compose.yml`)

# --rm               - флаг, чтобы удалить контейнер после использования
# --service-ports    - флаг, чтобы использовать порт заданный в сервисе в `docker-compose.yml`, в новый контейнер (По умолчанию при использовании `run` порты не пробрасываются)
# -d или --detach    - флаг запускает контейнер в фоновом режиме
...
# $ docker compose run web rake db:create                - создать в Рэилс базу данных в контейнере сервиса run
# $ docker compose run -f docker-compose.dev.yml web rake db:create  - тоже что и выше, если docker-compose-фаила, нестандартное
# $ docker compose run --rm --service-ports ruby_dev     - запустит терминал bash, который станет средой разработки Rails, где будут доступны команды Rails. ruby_dev зто название сервиса, указанных в нашем файле docker-compose.yml


# 4. exec - если нужно работать внутри терминала запущенного контейнера, то можно открыть новый терминал и выполнить эту команду. Эта команда откроет оболочку внутри выбранного контейнера, и позврлит запускать, например команды Реилс приложения как `rails console`, `rails generate` итд.

# (??? --service-ports  ???  проброситиь порты для Rspec Capibara)

# $ docker compose web bash                            - работать с терминалом bash внутри контейнера (запустить в новом терминале)
# $ docker compose -f docker-compose.dev.yml web bash  - тоже что и выше, если docker-compose-фаил с нестандартнм именем

# $ exit                                               - выйти из внутреннего терминала контейнера


# 5. down - команда очистки, Docker удалит все ваши настройки и загруженные образы.

# $ docker compose down        - остановить все контейнеры запущенные сервисами
