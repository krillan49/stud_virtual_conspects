...
# Docker Compose - это надстройка над Докером, если нам нужно запустить множество приложений, позволяет поместить в отдельный фаил все, что мы прописываем отдельными командами, настройки, образы, порты и потом просто запустить его одной командой. Позволяет запускать многоконтейнерные Docker-приложения
...
# $ docker-compose up --build  - запуск всех сервисов, с пересборкой образов контейнеров перед запуском(удобно использовать вместо 2х команд build и up)
# --build   - флаг чтобы пересобрать образы перед тем, как запустить их. Полезно, если внесли изменения в код приложения или Dockerfile
...
# В одной директории можно разместить несколько Dockerfile и docker-compose.yml файлов, различая их по именам. Это позволит использовать разные конфигурации для разработки и продакшен-окружения

# Dockerfile.dev  - например для разработки
# Dockerfile.prod - например для продакшена

# docker-compose.dev.yml  - для разработки будет использовать Dockerfile.dev
